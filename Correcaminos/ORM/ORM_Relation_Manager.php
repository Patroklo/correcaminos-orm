<?php namespace Correcaminos\ORM;
	...
	class ORM_Relation_Manager {
		...
		function __construct($join_data)
		{
			...
			{
				$this->relationObject = new ORM_Relation_ManyToMany($join_data);
			}
		}
	...
	class ORM_Relation_ManyToMany extends ORM_Relation {
		...
		public function get_data()
		{
			...
			{
				if($co['Field'] != $this->join_data['intermediatereferencedColumnName'] and $co['Field'] != $this->join_data['intermediateColumnName'])
				{
					$col_list[] = $co['Field']; 
				}
			}
			...
			$data = $this->ORM_QueryBuilder->get_grouped($this->join_data['intermediateTable'].'.'.$this->join_data['intermediateColumnName']);
